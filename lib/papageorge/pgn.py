...
def test_pgn():
    from papageorge.board import TestCli
    ...
    for x in lines:
        for y in x:
            print('{}. {} {:04b} {} {}'.format(y.halfmove//2+1,y, y.castling, y.move, y.comment))
        print()
...
def test_AB():
    ...
    r = re.compile('^:\[Game (\d+)\] .+ PV: (?P<moves>.+)')
    txt = ':[Game 375] Depth: 16 Score: 4.71 PV: 1. ... Ne2+ 2. Kd2 Nxc3 3. Nxf6 Nxd1 4. Nxd7 Nxf2 5. Nxc5 Nxh1 6. Bxh1 Kg7 7. d4 Kf6 8. c4 Bf5 9. Ndxb7 Rc8'
    m = r.match(txt)
    m.group('moves')

    from papageorge.model import GameState
    ic = GameState('--KR---RPPP--PB---QP--P----n------pN----p----np--p-r---pr-bN---k')
    ic.turn = False
    ...
    for x in lines:
        for y in x:
            print('{}. {} {:04b} {}'.format(y.halfmove//2+1,y, y.castling, y.move))
        print()
